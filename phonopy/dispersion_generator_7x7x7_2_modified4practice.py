...
#sys.exit()
suffix = '_' + str(init_no) + '_' + str(ndisps) #Added newly
directions = ['_G_X']

for a in range(1, 6):
    fc_per_ts_df[0] = a + (fc_per_ts_df[2].multiply(8).add(fc_per_ts_df[4].multiply(2)).add(fc_per_ts_df[5].multiply(4)).add(fc_per_ts_df[8].multiply(4)).add(fc_per_ts_df[9].multiply(8)).add(fc_per_ts_df[14].multiply(8)).add(fc_per_ts_df[15].multiply(16)).add(fc_per_ts_df[18].multiply(8))).multiply(-1)
    fc_per_ts_df[1] = fc_per_ts_df[0]
    
    out_filenames = [] # Newly added
    
    for direction in directions:
        out_filenames.append(direction+suffix+'.csv')
    freq_dict = {}
    freq_dict_G_X = {} # Blank dictionary added newly
...
# for m in left:
 ...
 #        print(root, m, )
        ...
        distance = phonons[0]['distance']
        print(len(phonons))
        band_contents = phonons[0]['band']
        freqs = []
        for j in range(6):
            freqs.append(band_contents[0]['frequency'])
        print(freqs[0], a)
           
        if m == init_no: # if file_number == init_file_number:
            freq_dict_G_X[distance] = freqs
        else:
            freq_dict_G_X[distance].extend(freqs)
    
    df_G_X = pd.DataFrame(freq_dict_G_X) #Newly added
    freq_dfs = [df_G_X]

    for c, out_file in enumerate(out_filenames):
        freq_dfs[c].to_csv(out_path+root+out_file)
sys.exit()
