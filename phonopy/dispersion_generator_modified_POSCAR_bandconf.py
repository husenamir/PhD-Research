...
system_size = 5
#Reading data as df
df = pd.read_csv("/Users/amirhusen/Desktop/Amir/Research/phonopy/2_6_50/data.csv")
for row in range(df.shape[0]):

    mass = df.iloc[row, 0]
    a_val = df.iloc[row,1] #Side length of each cube in bcc
    alat = a_val * system_size # the size of the box
    
    band_conf_path = "/Users/amirhusen/Desktop/Amir/Research/phonopy/2_6_50/band.conf"
    

    # --- Modify band.conf ---
    with open(band_conf_path, "r") as f:
        band_lines = f.readlines()

    # Replace second line (index 1)
    #if "MASS" in band_lines[1].upper():
    band_lines[1] = f"MASS = {mass:.2f}\n"

    # Save updated file
    with open(band_conf_path, "w") as f:
        f.writelines(band_lines)

    
    poscar_path = "/Users/amirhusen/Desktop/Amir/Research/phonopy/2_6_50/POSCAR"
    # --- Modify POSCAR ---
    with open(poscar_path, "r") as f:
        poscar_lines = f.readlines()

    # Replace second row (index 1) with "2.5"
    poscar_lines[1] = f"{a_val}\n"

    # Save updated file
    with open(poscar_path, "w") as f:vi 
    ...
    [α0, α1, β1, α2, β2] = [ df.iloc[row,2], df.iloc[row,3], df.iloc[row,4], df.iloc[row,5], df.iloc[row,6]]
    α3 = 0#0.0535625 
    β3 = 0#-0.0975
    γ3 = 0#-0.06625
    α4 = 0#-0.0473125
    ...
    for ax in axes:
        ...
        ax.plot(distances, freqs1, color='red')
        ax.plot(distances, freqs2, color='red')
        ax.plot(distances, freqs3, color='red')
    
        # Style
        ax.set_ylim(-2, 10)
        ax.set_xlabel('q-points', labelpad=17)
        ...
        # Custom text and ticks
        for x, label in zip(x_label, labels):
            ax.text(x, y_label, label, ha='center', va='top', fontsize=10)
    
        for x, point in zip(x_label, points):
            ax.text(x, -2.3, point, ha='center', va='top', fontsize=10)
    
        for x in x_label:
            ax.vlines(x, -2.2, -1.8, color='black', linewidth=1)
...
""" 
    ...
    plt.xlabel('q-points', labelpad=17, fontsize=12)
    ...
    x_label = [distances[0], distances[100], distances[401], distances[602], distances[803]]
    y_label = 10.7
    ...
    plt.text(distances[25], 9.8, f'm = {mass}', ha='center', va='top', fontsize=12)
    plt.text(distances[25], 9.0, f'a = {a_val}', ha='center', va='top', fontsize=12)      

    for x, label in zip(x_label, labels):
        plt.text(x, y_label, label, ha='center', va='top', fontsize=12)
        
    for x, point in zip(x_label, points):
        plt.text(x, -2.3, point, ha='center', va='top', fontsize=12)
        
    for x in x_label:
        plt.vlines(x, -2.2, -1.8, color='black', linewidth=1)   
    
    
    plt.savefig('/Users/amirhusen/Desktop/Figures/'+f'{mass}'+'_'+f'{a_val}'+'.png')
